...
class Diffusion(object):
    ...
    def train(self):
        ...
        if self.args.resume_training:
            ...
            if self.config.data.dataset == "CIFAR10":
                ...
                ckpt = './instanceData/ddim/saved/celeba_ckpt.pth'
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if step % self.config.training.snapshot_freq == 0 or step == 1:
                    ...
                    torch.save(
                        states,
                        os.path.join(self.args.checkpoint_path, "ckpt_{}.pth".format(step)),
                    )
                    torch.save(states, os.path.join(self.args.checkpoint_path, "ckpt.pth"))
    ...
    def sample(self):
        ...
        if not self.args.use_pretrained:
            if getattr(self.config.sampling, "ckpt_id", None) is None:
                states = torch.load(
                    os.path.join(self.args.checkpoint_path, "ckpt.pth"),
                    map_location=self.config.device,
                )
            else:
                states = torch.load(
                    os.path.join(
                        self.args.checkpoint_path, f"ckpt_{self.config.sampling.ckpt_id}.pth"
                    ),
                    map_location=self.config.device,
                )
            model = model.to(self.device)
    ...
    def sample_fid_bd(self, model):
        ...
        with torch.no_grad():
            for _ in tqdm.tqdm(
                ...
                ### Here the mean of attack is shifted by miu, and variance is changed to Sigma_b
                x = self.args.gamma * x + miu  # N(miu,Sigma_b)
    ...
    def sample_sequence(self, model):
        ...
        for i in range(x.shape[1]):
            tvu.save_image(x[1:, i], os.path.join('./instanceData', f"img{i}_process_benign.png"),
                           nrow=25)  # [100,3,32,32]
            #for j in range(x.shape[0]):
            #    tvu.save_image(
            #        x[j, i], os.path.join('./instanceData/images/original', f"img{i}_timestep{j}.png")
                    # [3,32,32]
           #     )
    ...
    def sample_sequence_bd(self, model):
        ...
        for i in range(x.shape[1]):
            tvu.save_image(x[1:, i], os.path.join('./instanceData', f"img{i}_process_trojaned.png"),
                           nrow=25)  # [100,3,32,32]
            # for j in range(x.shape[0]):
            #     tvu.save_image(
            #         x[j,i], os.path.join('./instanceData/ddim/images/d2din', f"img{i}_timestep{j}.png")#[3,32,32]
            #     )
